...
### 5.6 ProtocolHandler与DebugChain的接口契约（设计规划，LT-QT-001修正）

⚠️ **本节为设计规划，当前代码未实现**

**与架构文档对齐说明（LT-QT-001修正）**：
- 对照架构设计文档第12节（可扩展性设计）
...
#### 阶段3分步实施计划（LT-LD-003新增）

**阶段3目标**：完成Connection与ProtocolHandler的联调，实现TLS握手

| 步骤 | 步骤名称 | 具体任务 | 验收标准 | 依赖 |
|-----|---------|---------|---------|------|
| 3.1 | ConfigConverter实现 | 1. 创建ConfigConverter类<br/>2. 实现convert_cert_config()<br/>3. 实现convert_tls_config()<br/>4. 编写单元测试 | 单元测试通过，配置正确转换 | 无 |
| 3.2 | ProtocolHandler初始化 | 1. Http1Handler::init()获取Buffer指针<br/>2. 设置Buffer到TlsHandler<br/>3. 保存cert_config和tls_config<br/>4. 初始化TLS握手 | init()返回0，内部状态正确 | 3.1 |
| 3.3 | IoThread ACCEPT handler | 1. 在IoThread中定义ACCEPT handler<br/>2. 实现ListenInfo传递机制<br/>3. 创建Connection和ProtocolHandler<br/>4. 调用ProtocolHandler::init() | 能接受TCP连接，创建Connection对象 | 3.2 |
| 3.4 | READ事件handler | 1. 定义READ事件handler<br/>2. 调用ProtocolHandler::on_read()<br/>3. 处理-EAGAIN返回值<br/>4. 错误时关闭连接 | READ事件能触发on_read()调用 | 3.3 |
| 3.5 | TLS握手端到端 | 1. 完整TLS握手流程<br/>2. 状态转换：ACCEPTING→TLS_HANDSHAKING→CONNECTED<br/>3. 握手完成验证 | 能完成TLS握手，状态转为CONNECTED | 3.4 |
