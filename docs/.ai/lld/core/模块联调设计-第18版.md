...
**创建日期**: 2026-02-20
**状态**: 修正版（第17轮）
**重要说明**: 本文档区分「已实现」和「设计规划」两部分内容，设计规划模块（DebugChain、Callback）已明确标注
...
- **虚线灰色箭头**：表示设计规划的依赖关系（当前代码不存在，为设计规划）
- **灰色背景 + ⚠️ [设计规划] 标记**：表示设计规划模块，当前代码不存在

**说明**：
- Callback和DebugChain与Server的依赖为设计规划，当前代码不存在
...
- **原方案**：作为Server私有方法
- **修正方案**：使用独立的ConfigConverter类（作为独立的Adaptor层，不属于Protocol模块核心）
- **模块边界说明**：
  - Config模块：提供源配置结构（`config::CertificatesConfig`）
  - Protocol模块：提供目标配置结构（`protocol::CertConfig`）和协议处理逻辑
  - ConfigConverter：**独立适配层**，负责Config→Protocol的配置转换，不包含协议处理逻辑
- **理由**：
...
```cpp
// ==========================================
// ConfigConverter独立类（Adaptor层）
// ⚠️ [设计规划] - 当前代码中不存在
// 【模块边界说明】：本类是Config模块与Protocol模块之间的适配层，
//                  不属于Protocol模块的核心协议处理逻辑
// 实现位置：建议放在 独立的adaptor模块 或 protocol模块的adaptor子目录
...
**注意**: HTTP/2和国密支持在当前代码中不存在，为设计规划，暂不列入联调阶段。
...
⚠️ **阶段4-6为设计规划，当前代码不存在，Checklist已精简**
...
| 设计规划 | 设计规划 | 本文档中标记为⚠️ [设计规划]的内容表示当前代码不存在，为设计规划 |
...
- **v18新增 - 单点修正1**: 统一"设计规划"与"未实现"表述，全文使用"设计规划"描述当前代码不存在的内容，避免术语混用
- **v18新增 - 单点修正2**: 明确ConfigConverter的职责边界，补充模块边界说明，强调ConfigConverter是独立的Adaptor层，不属于Protocol模块的核心协议处理逻辑
