...
- `https_server_sim::config::Config` - Config类位于config子命名空间
...
- Config (https_server_sim::config::Config)
- Utils
...
- ACCEPT事件的handler在IoThread内部定义，在EventLoop中执行，直接调用ConnectionManager创建连接
...
- **add_listen_fd接口说明**：`add_listen_fd(fd, port)` 仅接受fd和port两个参数，不接受handler参数，事件handler由IoThread内部设置
...
- **调用时机说明**:
  - `handle_accept()`: 由IoThread内部定义的ACCEPT事件handler调用
  - `create_protocol_handler()`: 在handle_accept()中调用，用于创建ProtocolHandler
  - `convert_cert_config()`: 在handle_accept()中调用，用于转换证书配置
  - `convert_tls_config()`: 在handle_accept()中调用，用于转换TLS配置

**server.hpp 新增私有方法声明示例**：
```cpp
// 在 codes/core/server/include/server/server.hpp 的 private 区域添加：
private:
    // ⚠️ [设计规划] 以下方法为新增私有方法
    void handle_accept(int fd, uint16_t server_port);
    std::unique_ptr<https_server_sim::protocol::ProtocolHandler>
        create_protocol_handler(const https_server_sim::config::Config& config);
    void convert_cert_config(
        const https_server_sim::config::CertificatesConfig& src,
        https_server_sim::protocol::CertConfig& dst);
    void convert_tls_config(
        const https_server_sim::config::Config& config,
        https_server_sim::protocol::TlsConfig& dst);
```

```cpp
// ==========================================
// Server::start() 中的实现（仅add_listen_fd，不注册handler）
// ⚠️ [重要] add_listen_fd仅接受fd和port两个参数，不接受handler
// 事件handler由IoThread内部设置
// ==========================================
...
int https_server_sim::server::Server::start() {
    ...
    // 仅注册listen fd和port到MsgCenter
    // 事件handler由IoThread内部自动设置，不由Server直接注册
    for (size_t i = 0; i < listen_fds_.size(); ++i) {
        int fd = listen_fds_[i];
        uint16_t port = listen_ports_[i];

        // add_listen_fd仅接受fd和port两个参数
        ret = msg_center_->add_listen_fd(fd, port);
        // 注：IoThread内部会自动设置ACCEPT事件handler
        // handler会通过某种方式获取Server的上下文并调用handle_accept()
    }
...
- **实现位置**: `codes/core/server/source/server.cpp`
- **命名空间结构说明**:
  - `https_server_sim::server::Server` - Server类位于此命名空间
  - `https_server_sim::config::Config` - Config类位于此命名空间
  - `https_server_sim::protocol::ProtocolHandler` - ProtocolHandler类位于此命名空间
  - 转换函数中使用 `config::` 前缀访问Config模块类型，使用 `protocol::` 前缀访问Protocol模块类型
...
**文档版本**: v15
**修订说明**:
- LT-001: 修正 add_listen_fd 接口描述，明确仅接受fd和port，事件handler在IoThread内部设置
- LT-002: 补充设计规划接口的实现指导，明确server.hpp中private区域的新增方法声明和调用时机
- LT-003: 修正 transition_to() 参数说明
- LT-004: 修正时序图中Event.handler的定义位置说明
- LT-005: 补充命名空间嵌套关系的明确说明
