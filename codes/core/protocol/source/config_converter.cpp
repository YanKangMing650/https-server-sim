    ...
    CertConfig& dst)
{
    ...
    // 国密双证书字段（直接映射）
    dst.sm2_cert_path = src.sm2_cert_path;
    dst.sm2_key_path = src.sm2_key_path;
    dst.sm2_sign_cert_path = src.sm2_sign_cert_path;
    dst.sm2_sign_key_path = src.sm2_sign_key_path;
    ...
    dst.use_gmssl = !src.sm2_cert_path.empty() || !src.sm2_key_path.empty() ||
                    !src.sm2_sign_cert_path.empty() || !src.sm2_sign_key_path.empty();
}
